...
void Player::AttachToPhysics(const glm::vec3 &spawn)
{
	...
	desc.position = PxExtendedVec3(spawn.x, spawn.y - EYE_LEVEL,
				       spawn.z); // foot pos
	...
	if (!m_Controller)
		throw std::runtime_error(
			"PhysX: failed to create CapsuleController");

	// Start camera exactly above the controller feet, at eye height.
	p_Position = spawn;
	*(glm::vec3 *)(&p_Camera.GetPosition()) =
		p_Position; // yes, still ugly. Fix your camera API later.
	p_Camera.Refresh();

	// Voxel bridge window — don't be silly big. Keep it tight and cheap.
	m_BlockBridge = std::make_unique<BlockSceneBridge>(
		px.Physics(), px.Scene(), px.DefaultMaterial());
	m_BlockBridge->UpdateNeighborhood(glm::ivec3((int)floor(spawn.x),
						     (int)floor(spawn.y),
						     (int)floor(spawn.z)),
					  glm::ivec3(12, 8, 12));
...
glm::vec3 Player::ComputeWalkDir() const
{
	...
	return glm::normalize(walk_forward) +
	       glm::normalize(walk_right) *
		       0.0f; // placeholder; caller composes
}
...
void Player::UpdateFreeFly(GLFWwindow *window, float camera_speed)
{
	...
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
		p_Camera.ApplyAcceleration(forward * camera_speed);
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
		p_Camera.ApplyAcceleration(back * camera_speed);
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
		p_Camera.ApplyAcceleration(-(right * camera_speed));
...
void Player::UpdatePhysX(GLFWwindow *window, float camera_speed, float dt)
{
	...
	glm::ivec3 center{ (int)floor(camPos.x),
			   (int)floor(camPos.y - EYE_LEVEL),
			   (int)floor(camPos.z) };
	...
	{
		float yaw = glm::radians(p_Camera.GetYaw());
		const glm::vec3 walk_forward =
			glm::vec3(cosf(yaw), 0.0f, sinf(yaw));
		const glm::vec3 walk_right =
			glm::vec3(-sinf(yaw), 0.0f, cosf(yaw));
		if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
			move += walk_forward;
		if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
			move -= walk_forward;
	...
	constexpr float gravity =
		-20.0f; // blocks/s^2. More snappy than Earth g in “block units”.
...
void Player::OnUpdate(GLFWwindow *window)
{
	...
	if (dt <= 0.0f)
		dt = 1.0f / 60.0f;
	if (dt > 0.1f)
		dt = 0.1f; // don't explode on stalls
	m_TimeLast = tNow;
...
void Player::OnEvent(EventSystem::Event e)
{
	if (e.type == EventSystem::EventTypes::MouseScroll) {
		if (e.msy > 0.0f) {
			if (p_Camera.GetFov() < 71.0f)
				p_Camera.SetFov(p_Camera.GetFov() + 0.1f);
		} else if (e.msy < 0.0f) {
			if (p_Camera.GetFov() > 69.50f)
				p_Camera.SetFov(p_Camera.GetFov() - 0.1f);
		}
...
bool Player::TestBlockCollision(const glm::vec3 &position)
{
	...
	struct NoSelf final : PxQueryFilterCallback {
		PxRigidActor *self;
		explicit NoSelf(PxRigidActor *a)
			: self(a)
		{
		}

		PxQueryHitType::Enum
		preFilter(const PxFilterData &, const PxShape * /*shape*/,
			  const PxRigidActor *actor,
			  PxHitFlags & /*queryFlags*/) override
		{
			return (actor == self) ? PxQueryHitType::eNONE :
