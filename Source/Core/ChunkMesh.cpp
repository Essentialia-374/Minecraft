...
GLClasses::IndexBuffer &quad_index_buffer()
{
	...
	if (!inited) {
		inited = true;
		const size_t max_quads = size_t(CHUNK_SIZE_X) * CHUNK_SIZE_Y *
					 CHUNK_SIZE_Z * 6; // worst case
		...
		ibo.BufferData(GLsizeiptr(idx.size() * sizeof(GLuint)),
...
inline void setup_vao(GLClasses::VertexArray &vao, GLClasses::VertexBuffer &vbo)
{
	...
	vbo.VertexAttribIPointer(0, 3, GL_UNSIGNED_BYTE, sizeof(Vertex),
				 (void *)offsetof(Vertex, position));
	vbo.VertexAttribIPointer(1, 2, GL_UNSIGNED_SHORT, sizeof(Vertex),
				 (void *)offsetof(Vertex, texture_coords));
	vbo.VertexAttribIPointer(2, 1, GL_UNSIGNED_BYTE, sizeof(Vertex),
				 (void *)offsetof(Vertex, lighting_level));
	vbo.VertexAttribIPointer(3, 1, GL_UNSIGNED_BYTE, sizeof(Vertex),
				 (void *)offsetof(Vertex, block_face_lighting));
	vao.Unbind();
}

struct Neighborhood {
	ChunkDataTypePtr c, l, r, f,
		b; // center, left(-x), right(+x), forward(+z), backward(-z)
...
inline bool face_visible(const Block &cur, const Block *nb)
{
	if (cur.IsTransparent()) {
		...
		if (!nb)
			return true;
		return nb->IsTransparent() &&
		       nb->p_BlockType != cur.p_BlockType;
	} else {
...
ChunkMesh::ChunkMesh()
	...
	, m_TransparentVBO(GL_ARRAY_BUFFER)
	, m_ModelVBO(GL_ARRAY_BUFFER)
{
...
bool ChunkMesh::ConstructMesh(Chunk *chunk, const glm::vec3 &chunk_pos)
{
	...
	Neighborhood nb{
		ChunkData,
		_GetChunkDataForMeshing(static_cast<int>(chunk_pos.x - 1),
					static_cast<int>(chunk_pos.z)),
		_GetChunkDataForMeshing(static_cast<int>(chunk_pos.x + 1),
					static_cast<int>(chunk_pos.z)),
		_GetChunkDataForMeshing(static_cast<int>(chunk_pos.x),
					static_cast<int>(chunk_pos.z + 1)),
		_GetChunkDataForMeshing(static_cast<int>(chunk_pos.x),
					static_cast<int>(chunk_pos.z - 1)),
		ChunkLData,
		_GetChunkLightDataForMeshing(static_cast<int>(chunk_pos.x - 1),
					     static_cast<int>(chunk_pos.z)),
		_GetChunkLightDataForMeshing(static_cast<int>(chunk_pos.x + 1),
					     static_cast<int>(chunk_pos.z)),
		_GetChunkLightDataForMeshing(static_cast<int>(chunk_pos.x),
					     static_cast<int>(chunk_pos.z + 1)),
		_GetChunkLightDataForMeshing(static_cast<int>(chunk_pos.x),
					     static_cast<int>(chunk_pos.z - 1))
	};
	...
	for (int x = 0; x < CHUNK_SIZE_X; ++x) {
		for (int y = 0; y < CHUNK_SIZE_Y; ++y) {
			for (int z = 0; z < CHUNK_SIZE_Z; ++z) {
				...
				float base_light =
					ChunkLData->at(x).at(y).at(z);
				if (y >= 0 && y < CHUNK_SIZE_Y - 1)
					base_light =
						ChunkLData->at(x).at(y + 1).at(
							z);
				if (blk.IsModel()) {
					AddModel(chunk, local_position,
						 blk.p_BlockType, base_light);
					continue;
				}
				...
				for (const auto &d : Dirs) {
					...
					const int nz = z + d.dz;
					const Block *nbp =
						get_block_ptr(nb, nx, ny, nz);
					if (!face_visible(blk, nbp))
						continue;
					const uint8_t light =
						get_light(nb, nx, ny, nz);
					// The last arg is "buffer": in the old code 'true' meant opaque stream.
					AddFace(chunk, d.face, local_position,
						blk.p_BlockType, light,
						/*buffer=*/opaque);
				}
			}
		}
	...
	if (!m_Vertices.empty()) {
		m_VBO.BufferData(GLsizeiptr(m_Vertices.size() * sizeof(Vertex)),
	...
	if (!m_TransparentVertices.empty()) {
		m_TransparentVBO.BufferData(
			GLsizeiptr(m_TransparentVertices.size() *
				   sizeof(Vertex)),
			m_TransparentVertices.data(), GL_STATIC_DRAW);
		p_TransparentVerticesCount =
			static_cast<uint32_t>(m_TransparentVertices.size());
		m_TransparentVertices.clear();
	}

	if (!m_ModelVertices.empty()) {
		m_ModelVBO.BufferData(
			GLsizeiptr(m_ModelVertices.size() * sizeof(Vertex)),
			m_ModelVertices.data(), GL_STATIC_DRAW);
		p_ModelVerticesCount =
			static_cast<uint32_t>(m_ModelVertices.size());
		m_ModelVertices.clear();
	}
...
void ChunkMesh::AddFace(Chunk *chunk, BlockFaceType face_type,
			const glm::vec3 &position, BlockType type,
			uint8_t light_level, bool buffer)
{
	glm::vec4 translation = glm::vec4(
		position,
		0.0f); // Model matrix for cubes is theatrical. Translation is enough.
	...
	const std::array<uint16_t, 8> &TextureCoordinates =
		BlockDatabase::GetBlockTexture(type, face_type);

	if (reverse_texture_coordinates) {
		v1.texture_coords = { TextureCoordinates[6],
				      TextureCoordinates[7] };
		v2.texture_coords = { TextureCoordinates[4],
				      TextureCoordinates[5] };
		v3.texture_coords = { TextureCoordinates[2],
				      TextureCoordinates[3] };
		v4.texture_coords = { TextureCoordinates[0],
				      TextureCoordinates[1] };
	}

	else {
		v1.texture_coords = { TextureCoordinates[0],
				      TextureCoordinates[1] };
		v2.texture_coords = { TextureCoordinates[2],
				      TextureCoordinates[3] };
		v3.texture_coords = { TextureCoordinates[4],
				      TextureCoordinates[5] };
		v4.texture_coords = { TextureCoordinates[6],
				      TextureCoordinates[7] };
	}
...
void ChunkMesh::AddModel(Chunk *chunk, const glm::vec3 &local_pos,
			 BlockType type, float light_level)
{
	...
	for (int i = 0; i < model.p_ModelVertices.size(); i++) {
		...
		glm::vec4 pos =
			glm::vec4(model.p_ModelVertices.at(i).position, 1.0f);
		pos = translation * pos;
